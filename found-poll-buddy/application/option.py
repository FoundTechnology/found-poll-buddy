...
class Option:
    ...
    def author_id(self) -> Optional[int]:
        """
        The ID of the member who added the option, or None if
        the option existed from poll creation.
        """
    ...
    def remove_vote(self, voter_id: int):
        """
        Remove a vote from the given user on this option.
        If no such vote exists, nothing happens.

        This method does not remove the vote from the database.
        To remove the vote from the database, use the `delete_vote` method.
    ...
    def delete_vote(self, voter_id: int):
        ...
        This method deletes the vote from the database.
        To remove the vote from the option without affecting the database
        (for example if the vote has already been removed from the database),
        use the `remove_vote` method instead.

        Args:
            voter_id: The ID of the user whose vote is to be deleted.
        """
        asyncio.create_task(
            data.cruds.votes_crud.delete_users_votes_from_option(self.option_id, voter_id)
        )
        self.remove_vote(voter_id)

    def add_vote(self, voter_id: int):
        """
        Add a vote from the given user on this option.
        If such a vote already exists, nothing happens.
        If the poll cannot have more than one vote per user,
        all other votes from this user are removed.
    ...
    def toggle_vote(self, voter_id: int):
        """
        Toggle a user's vote on this option. If adding their
        vote would cause too many votes from the same user,
        the rest of their votes are removed.
    ...
    async def create_option(
        ...
        Args:
            ...
            author_id: The ID of the member who created the option, or None,
            if the option is being created at poll creation.
    ...
    async def create_options(
        ...
        Args:
            ...
            author_id: The ID of the person who added this option,
            or None if the options were created at the time of creation.

...
class PollsCrud(Crud):
    ...
    async def add(self, poll: "Poll") -> int:
        ...
        await self.__insert_permissions("allowed_vote_viewers", poll.allowed_vote_viewers, poll_id)
        await self.__insert_permissions("allowed_editors", poll.allowed_editors, poll_id)
    ...
    async def __insert_permissions(self, table: str, mentions: Iterable[Mention], poll_id: int):
    ...
    async def fetch_all(self) -> set["Poll"]:
        ...
        for r in records:
            poll = Poll(
                ...
                (Mention(mention[0], mention[1]) for mention in r["allowed_vote_viewers"]),

...
async def one(
    ...
    **fields,
    ...
    results = await many(pool, table, keys, (values,), on_conflict=on_conflict, returning=returning)
...
async def many(
    ...
    *,
    ...
    values = ", ".join(f"({", ".join(next(placeholders) for _ in columns)})" for _ in records)
    if values == "":
        return []

...
class SeeOptionResultsEmbed(Embed):
    ...
    def split(self) -> list[Embed]:
        LIMIT = (
            5700  # 6000 actually, but just to be safe and accounting for the title and vote count
        )
        if len(self) < LIMIT:
            return [self]
        ...
        while end < len(desc):
            ...
            embeds.append(Embed(color=self.colour, title=self.title, description=desc[start:end]))
            start = end + 1
        return embeds

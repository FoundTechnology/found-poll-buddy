...
class Paul(InteractionBot):
    def __init__(self, *args, **kwargs):
        ...
        async def poll(
            ...
            options: Iterable[str] = Param(
                desc=(
                    "Separate each option with a pipe (|). By default the options are" " yes or no."
                ),
            ...
            await inter.response.send_message(
                embed=PollEmbedBase(question, "<a:loading:904120454975991828> Loading poll...")
            )
    ...
    async def new_poll(self, params: PollCommandParams, author_id: int, message: Message):
        try:
    ...
    async def add_poll_option(self, poll: Poll, label: str, author_id: int, inter: ModalInteraction):
    ...
    async def __update_poll_message(self, poll: Poll, message: Optional[Message] = None):
        ...
        try:
            ...
            logger.exception(f"couldn't update poll {poll.question} in {poll.channel_id}.")
        except Exception:
            logger.exception(f"couldn't update poll {poll.question}")

    async def __get_poll_message(self, poll: Poll) -> Message:
        return await self.get_partial_messageable(poll.channel_id).fetch_message(poll.message_id)
